...
export const Tamper = {
  ...
  unpackExistence(element, defaultAttrs = {}) {
    ...
    const processBitArray = (array) => {
      ...
      throw new Error(`Unrecognized control code: ${cc}`);
    };
  ...
  unpackIntegerEncoding(element, numItems) {
    ...
    const getPossibility = (i) => (i === 0 ? null : element.possibilities[i - 1]);
    const output = [];

    for (let i = 0; i < numItems; i += 1) {
      const bitWindow = bitArray.slice(i * itemWindowWidth, (i * itemWindowWidth) + itemWindowWidth);
      for (let j = 0; j < itemChunks; j += 1) {
        const choice = bitWindow.slice(j * bitWindowWidth, (j * bitWindowWidth) + bitWindowWidth);
  ...
  unpackData(data, defaultAttrs = {}) {
    ...
    return (data.collection || []).map((item) => extend(clone(defaultAttrs), item));
  }
};
