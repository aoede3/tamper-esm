...
function createEncoder(env) {
  class Bitpusher2 {
    ...
    slice(begin, end) {
      const bitpusher = new Bitpusher2();
  ...
  class IntegerPack2 extends Pack2 {
    static {
    ...
    initializePack(maxGuid, numItems) {
      this.baseOffset = IntegerPack2.HEADER_OCTETS * 8;
  ...
  class BitmapPack2 extends Pack2 {
    static {
    ...
    initializePack(maxGuid, numItems) {
      this.baseOffset = BitmapPack2.HEADER_OCTETS * 8;
  ...
  class ExistencePack2 extends Pack2 {
    static {
    ...
    finalizePack() {
      ...
      for (const cc of this.controlCodes) {
        if (cc.type === "keep") {
          ...
          env.writeUInt8(this.buffer, ExistencePack2.KEEP, offset);
          ...
          env.writeUInt8(this.buffer, ExistencePack2.SKIP, offset);
          env.writeUInt32BE(this.buffer, cc.offset, offset + 1);
          offset += 5;
        } else if (cc.type === "run") {
          env.writeUInt8(this.buffer, ExistencePack2.RUN, offset);
          env.writeUInt32BE(this.buffer, cc.offset, offset + 1);
          offset += 5;
        }
      }
    ...
    controlCode(cmd, offset = 0) {
      ...
      switch (cmd) {
        case "keep": {
          ...
          env.writeUInt8(buffer, ExistencePack2.KEEP, 0);
        ...
        case "skip":
          ...
          env.writeUInt8(buffer, ExistencePack2.SKIP, 0);
          env.writeUInt32BE(buffer, offset, 1);
          break;
        case "run":
          buffer = env.createBuffer(5);
          env.writeUInt8(buffer, ExistencePack2.RUN, 0);
          env.writeUInt32BE(buffer, offset, 1);
          break;
        default:
  ...
  class PackSet2 {
    ...
    constructor(opts = {}) {
      this.meta = {};
      this.existencePack = new ExistencePack2();
      this.attrPacks = {};
      this.bufferedAttrs = {};
      this.meta = opts;
    }
    ...
    pack(data, opts = {}) {
      ...
      if (localOpts.guidAttr == null) {
        localOpts.guidAttr = PackSet2.DEFAULT_GUID_ATTR;
      }
    ...
    buildPack(opts = {}, items) {
      ...
      const guidAttr = opts.guidAttr || PackSet2.DEFAULT_GUID_ATTR;
  ...
  function createPack2(attrName, possibilities, maxChoices) {
    let PackConstructor;
    if (maxChoices * log2(possibilities.length) < possibilities.length) {
      PackConstructor = IntegerPack2;
    } else {
      PackConstructor = BitmapPack2;
    }
    return new PackConstructor(attrName, possibilities, maxChoices);
  }
  __name(createPack2, "createPack");
  function createPackSet2() {
    return new PackSet2();
  }
  __name(createPackSet2, "createPackSet");
  return {
    Bitpusher: Bitpusher2,
    createPackSet: createPackSet2,
    PackSet: PackSet2,
    createPack: createPack2,
    Pack: Pack2,
    IntegerPack: IntegerPack2,
    BitmapPack: BitmapPack2,
    ExistencePack: ExistencePack2
  };
}
...
// encoders/js/index.ts
var encoder = createEncoder(node_default);
var createPackSet = encoder.createPackSet;
var PackSet = encoder.PackSet;
var createPack = encoder.createPack;
var Pack = encoder.Pack;
var IntegerPack = encoder.IntegerPack;
var BitmapPack = encoder.BitmapPack;
var ExistencePack = encoder.ExistencePack;
var Bitpusher = encoder.Bitpusher;
